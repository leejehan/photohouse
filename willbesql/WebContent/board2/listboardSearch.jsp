<%@page import="willbe.question.action.boardListAction"%>
<%@ page language="java" contentType="text/html; charset=utf-8"
	pageEncoding="utf-8"%>
<%@ page import="java.sql.*, java.util.*"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ page import="willbe.question.db.boardDTO"%>
...
<!-- 타이틀메뉴 틀 끝 -->
	...
	<!-- ------------------------------------------ -->
	<div id="listHeader">
		<div id="listHeader_title">문의 사항</div>
		<hr size='1' noshade>
	</div>
	<div id="listBody">
					...
					<tr>
						...
						<c:choose>
							<c:when test="${fn:length(dto.board_title)>10}">							
								<td id="listTitle"><a href="write_output.qt?num=${dto.board_num }">${fn:substring(dto.board_title,0,9)}...</a></td>
							</c:when>
							<c:otherwise>
								<td id="listTitle"><a href="write_output.qt?num=${dto.board_num }">${dto.board_title }</a></td>
							</c:otherwise>
						</c:choose>
		...
		<div id="listBody_body">
			<center>
				...
				%>
				<a
					href="listboardSearch.qt?title=${title }&currentPage=<%=i%>&fp=<%=fp%>&lp=<%=lp%>">
					<%
						out.println(i);
					%>
				</a>
		...
		<div id="listBody_foot">
			<input type="text" id="SearchText" name="titleSearch"
				placeholder="제목을 입력하세요"> <input type="button"
				id="searchButton" name="search" value="검색" onclick="SearchTitle()">
		</div>
	</div>
	<div id="listFoot">
		<div id="regitButton">
			<a href="write.qt">등록</a>
		</div>
	</div>
	<script>
		...
		var titlePage = $("input[name=title]").val();
		...
		function pageDecrease() {
			...
			var url = "listboardSearch.qt?title=" + titlePage + "&currentPage="
					+ DecreasePage + "&fp=" + String(fp) + "&lp=" + String(lp);
			location.href = url;
		}
		...
		function pageIncrease() {
			...
			var url = "listboardSearch.qt?title=" + titlePage + "&currentPage="
					+ IncreasePage + "&fp=" + String(fp) + "&lp=" + String(lp);
			location.href = url;
		}
		...
		function SearchTitle() {
			...
			var url = "listboardSearch.qt?title=" + title;
			location.href = url;
		}
	</script>
</body>
