...
DataSource ds;
	...
	public ArrayList<boardDTO> boardlist(){		
		...
		try {
			...
			String sql="select * from questionBoard order by board_num desc";
			...
			while(rs.next()){
				...
				boardDTO dto=new boardDTO(board_num, board_title, member_id, board_date, board_readcount);
				dtoList.add(dto);			
			}			
	...
	public void write(String id, String title, String contents, String file) {		
		...
		try {
			...
			String sql = "insert into questionBoard (member_id, board_title, "
	...
	public boardDTO writeView(String num) {		
		...
		try {
			conn = ds.getConnection();
			String sql = "select * from questionBoard where board_num=?";
			pstmt = conn.prepareStatement(sql);
			...
			while (rs.next()) {
				...
				dto = new boardDTO(num, member_id, board_title, board_date, board_contents, board_readcount, board_file);				
			}
			sql = "update questionBoard set board_readcount=board_readcount+1 where board_num = ?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, Integer.parseInt(num));
			pstmt.executeUpdate();
		} catch (Exception e) {
	...
	public boolean idCheck(String id, String num) {
		...
		try {
			conn = ds.getConnection();
			String sql = "select member_id from questionBoard where board_num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, Integer.parseInt(num));
	...
	public void delete(String num) {
		...
		try{
			conn = ds.getConnection();
			String sql = "delete from questionBoard where board_num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, Integer.parseInt(num));
	...
	public void modify(String num, String title, String contents, String userFile) {
		...
		try{
			conn = ds.getConnection();
			String sql = "update questionBoard set board_title=?, board_contents=?, board_file=? where board_num=?";
			pstmt = conn.prepareStatement(sql);
	...
	public ArrayList<boardDTO> boardSearchList(String SearchTitle) {
		...
		try {
			conn=ds.getConnection();
			String sql="select * from questionBoard where board_title like ? order by board_num desc";
			pstmt=conn.prepareStatement(sql);	
			pstmt.setString(1, title);
	...
	public boardDTO pageSearchList(String SearchTitle) {
		...
		try {
			conn=ds.getConnection();
			String sql="select * from questionBoard where board_title like ?";
			pstmt=conn.prepareStatement(sql);	
			pstmt.setString(1, title);
