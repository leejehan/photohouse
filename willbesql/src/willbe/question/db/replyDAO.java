...
DataSource ds;
	...
	public ArrayList<replyDTO> replylist(int num){
		...
		try{
			...
			String sql="select * from questionReply where board_num=? order by reply_num";			
			...
			while(rs.next()){
				...
				replyDTO dto=new replyDTO(reply_num, board_num, member_id, reply_contents, reply_date);
				dtoList.add(dto);			
			}			
	...
	public void inputReply(String contents, String id, String num) {		
		...
		try {
			...
			String sql = "insert into questionReply (board_num, member_id, reply_contents, reply_date)"
					+" values(?, ?, ?, now())";
	...
	public void replyAllDelete(String num) {
		...
		try{
			conn = ds.getConnection();
			String sql = "delete from questionReply where board_num=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, Integer.parseInt(num));
	...
	public void replyDelete(String replyNum, String id) {
		...
		try{			
			conn = ds.getConnection();
			String sql = "select member_id from questionReply where reply_num=?";
			pstmt = conn.prepareStatement(sql);
			...
			if(member_id.equals(id)){				
				conn = ds.getConnection();
				sql = "delete from questionReply where reply_num=?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, Integer.parseInt(replyNum));
